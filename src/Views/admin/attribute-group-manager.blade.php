{{--
    ...
    Variables: $rows (ShopAttributeGroup paginator); $form, $editingId, $sortField, $sortDir (state).
--}}
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">

    {{-- Left: add / edit form --}}
    <x-gp247::card :title="gp247_language_render($editingId ? 'action.edit' : 'admin.product_attribute_group.add_new')">
        <form wire:submit="save" class="space-y-4">
            @include('gp247-admin::partials.store-scope-picker', ['testid' => 'attribute-group-store-select'])
            <x-gp247::input :label="gp247_language_render('admin.product_attribute_group.name')" name="name"
                wire:model="form.name" :error="$errors->first('form.name')" required />

            <div>
                <label for="type" class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-200">
                    {{ gp247_language_render('admin.product_attribute_group.type') }}
                </label>
            ...
            <div class="flex items-center justify-between border-t border-gray-200 pt-4 dark:border-gray-700">
                <x-gp247::button variant="secondary" wire:click="cancelEdit" data-testid="admin-attribute-group-form-cancel">{{ gp247_language_render($editingId ? 'admin.cancel' : 'admin.reset') }}</x-gp247::button>
    ...
    {{-- Right: list --}}
    <x-gp247::card :title="gp247_language_render('admin.product_attribute_group.list')">
        <div class="mb-3">
            <input type="search" wire:model.live.debounce.300ms="keyword" placeholder="{{ gp247_language_render('admin.product_attribute_group.name') }}"
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100">
        </div>
        ...
        <x-gp247::table :empty="$rows->isEmpty() ? gp247_language_render('admin.no_records') : null">
            <x-slot:head>
                <tr>
                    <th class="cursor-pointer px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400" wire:click="setSort('name')">
                        {{ gp247_language_render('admin.product_attribute_group.name') }} @if ($sortField === 'name')<span class="text-[10px]">{{ $sortDir === 'asc' ? '▲' : '▼' }}</span>@endif
                    </th>
                    <th class="cursor-pointer px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400" wire:click="setSort('type')">
                        {{ gp247_language_render('admin.product_attribute_group.type') }} @if ($sortField === 'type')<span class="text-[10px]">{{ $sortDir === 'asc' ? '▲' : '▼' }}</span>@endif
                    </th>
            ...
            @foreach ($rows as $row)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 {{ (string) $row->id === (string) $editingId ? 'bg-blue-100 border-l-4 border-blue-500 dark:bg-blue-900 dark:border-blue-500' : '' }}" wire:key="attribute-group-{{ $row->id }}">
                    ...
                    <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $row->type }}</td>
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-end gap-1">
                            <x-gp247::button size="sm" variant="ghost" wire:click="editRow('{{ $row->id }}')" data-testid="admin-attribute-group-list-edit"><i class="fas fa-edit"></i></x-gp247::button>
