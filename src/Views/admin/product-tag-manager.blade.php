...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2" data-testid="shop-admin-product-tag-manager">
    ...
    <x-gp247::card :title="gp247_language_render($editingId ? 'action.edit' : 'admin.product_tag.add_new')">
        <form wire:submit="save" class="space-y-4">
            ...
            <div class="flex items-center justify-between border-t border-gray-200 pt-4 dark:border-gray-700">
                <x-gp247::button variant="secondary" href="{{ gp247_route_admin('admin_product_tag.index') }}" wire:navigate>{{ gp247_language_render($editingId ? 'admin.cancel' : 'admin.reset') }}</x-gp247::button>
    ...
    <x-gp247::card :title="gp247_language_render('admin.product_tag.title')">
        ...
        <x-gp247::table :empty="$rows->isEmpty() ? gp247_language_render('admin.no_records') : null">
            ...
            @foreach ($rows as $row)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 {{ (string) $row->id === (string) $editingId ? 'bg-blue-50 dark:bg-blue-900/30' : '' }}" wire:key="product-tag-{{ $row->id }}" data-testid="shop-admin-product-tag-row">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-end gap-1">
                            <x-gp247::button size="sm" variant="ghost" href="{{ gp247_route_admin('admin_product_tag.edit', $row->id) }}" wire:navigate><i class="fas fa-edit"></i></x-gp247::button>
