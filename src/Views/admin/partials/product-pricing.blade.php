...
{{-- WHY: each block mirrors a Shop Config > Product toggle (US-SADM-005); a field
     turned off in config is not rendered here (ProductManager::productFieldEnabled). --}}
<div class="space-y-4">
    {{-- Promotion (product_promotion) --}}
    @if ($this->productFieldEnabled('product_promotion'))
    ...
    {{-- Dimensions (product_weight / product_length), minimum + date-available (product_available).
         minimum has no config toggle, so it is always shown. --}}
    <div class="grid grid-cols-2 gap-3">
        @if ($this->productFieldEnabled('product_weight'))
        ...
        @endif
        @if ($this->productFieldEnabled('product_length'))
        ...
        @if ($this->productFieldEnabled('product_available'))
