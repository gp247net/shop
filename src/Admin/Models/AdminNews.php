...
use GP247\Shop\Models\ShopNews;
use Cache;
use GP247\Shop\Models\ShopNewsDescription;
use GP247\Shop\Models\ShopNewsStore;
...
class AdminNews extends ShopNews
{
    ...
    public static function getListTitleAdmin($storeId = null)
    {
        ...
        if (gp247_config_global('cache_status') && gp247_config_global('cache_news')) {
            if (!Cache::has($storeCache.'_cache_news_'.gp247_get_locale())) {
                ...
                gp247_cache_set($storeCache.'_cache_news_'.gp247_get_locale(), self::$getListTitleAdmin);
            }
            return Cache::get($storeCache.'_cache_news_'.gp247_get_locale());
        } else {
            if (self::$getListTitleAdmin === null) {
                $data = self::join($tableDescription, $tableDescription.'.news_id', $table.'.id')
                ->where('lang', gp247_get_locale());
